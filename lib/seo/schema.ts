...
function generateServiceDescription(params: SchemaParams): string {
  ...
  if (city && brand && appliance) {
    return `Professional ${formatBrandName(brand)} ${formatApplianceName(appliance)} repair services in ${formatCityName(city)}, CO. Same-day service, upfront pricing.`;
  } else if (city && appliance) {
    ...
    return `Professional ${formatBrandName(brand)} ${formatApplianceName(appliance)} repair in the Denver Metro area. Experienced technicians.`;
  } else {
